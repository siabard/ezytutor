...
pub async fn get_course_details_db(
    ...
    let course_row: Option<Course> = sqlx::query_as!(
        Course,
...
pub async fn post_new_course_db(
    ...
    let course_row = sqlx::query_as!(Course, "insert into ezy_course (tutor_id, course_name, course_description, course_format, course_structure, course_duration, course_price, course_language, course_level) values ($1, $2, $3, $4, $5, $6, $7, $8, $9) returning tutor_id, course_id, course_name, course_description, course_format, course_structure, course_duration, course_price, course_language, course_level, posted_time ", new_course.tutor_id, new_course.course_name, new_course.course_description, new_course.course_format, new_course.course_structure, new_course.course_duration, new_course.course_price, new_course.course_language, new_course.course_level)
        .fetch_one(pool)
        .await?;
...
pub async fn delete_course_db(
    ...
    let course_row: PgDone = sqlx::query!(
...
pub async fn update_course_details_db(
    ...
    let course_row = sqlx::query_as!(Course, "UPDATE ezy_course set course_name = $1, course_description = $2, course_format = $3, course_structure = $4, course_duration = $5, course_price = $6, course_language = $7, course_level = $8 WHERE tutor_id = $9 and course_id = $10 returning tutor_id, course_id, course_name, course_description, course_duration, course_level, course_format, course_language, course_structure, course_price, posted_time ", name, description, format, structure, duration, price, language, level, tutor_id, course_id).fetch_one(pool).await;
